...
from semseg.metrics import Metrics, runningScore
...
def evaluate(model, dataloader, device):
    ...
    for sample in tqdm(dataloader):
        ...
        if sliding:
            ...
            preds1 = model(images,depths)
            preds = preds1.softmax(dim=1)
...
def main(cfg):
    ...
    for case in cases:
        from semseg.datasets.nyu import NYU
        ...
        model = eval(cfg['MODEL']['NAME'])(classes=dataset.n_classes)
        # resume_checkpoint = torch.load(str(model_path), map_location=torch.device('cpu'))
        msg = model.load_state_dict(torch.load(str(model_path), map_location='cpu'), strict=False)
        print(msg)

        model = model.to(device)
        sampler_val = None
        dataloader = DataLoader(dataset, batch_size=1, num_workers=eval_cfg['BATCH_SIZE'], pin_memory=False, shuffle=False, sampler=sampler_val)
        if True:
